...
def upgrade() -> None:
    ...
    op.execute("CREATE TRIGGER trg_orgs_updated_at BEFORE UPDATE ON orgs FOR EACH ROW EXECUTE FUNCTION set_updated_at();")
    ...
    op.execute("CREATE TRIGGER trg_org_members_updated_at BEFORE UPDATE ON org_members FOR EACH ROW EXECUTE FUNCTION set_updated_at();")
    ...
    op.execute("CREATE TRIGGER trg_farms_updated_at BEFORE UPDATE ON farms FOR EACH ROW EXECUTE FUNCTION set_updated_at();")
    ...
    op.execute("CREATE TRIGGER trg_fields_updated_at BEFORE UPDATE ON fields FOR EACH ROW EXECUTE FUNCTION set_updated_at();")
    ...
    op.create_table(
        ...
        sa.Column("org_id", UUID(as_uuid=True), sa.ForeignKey("orgs.id", ondelete="CASCADE"), nullable=False),
        sa.Column("field_id", UUID(as_uuid=True), sa.ForeignKey("fields.id", ondelete="CASCADE"), nullable=False),
        sa.Column("layer_id", UUID(as_uuid=True), sa.ForeignKey("raster_layers.id", ondelete="CASCADE"), nullable=False),
        sa.Column("date", sa.Date, nullable=False),
    ...
    op.execute("CREATE TRIGGER trg_alerts_updated_at BEFORE UPDATE ON alerts FOR EACH ROW EXECUTE FUNCTION set_updated_at();")
    ...
    op.execute("CREATE TRIGGER trg_scouting_updated_at BEFORE UPDATE ON scouting_observations FOR EACH ROW EXECUTE FUNCTION set_updated_at();")
